...
- The dot `.` is a path to the current working directory.
  ...
  directory. This path would be equivalent to just writing `lesson-plan.md`.
- The double-dot `..` is a path to the parent directory of the current working directory.
  For example, `../comp-302/notes.txt` is a relative path.
...
simplest kind of relative path is just a single entry name, no slashes anywhere at all.
