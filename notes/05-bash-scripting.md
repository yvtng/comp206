...
Here are some of the most useful `test` options on files:
...
- `N1 -ne N2`: if `N1` is not equal to `N2`
- `N1 -lt N2`: if `N1` is less than `N2`
- `N1 -le N2`: if `N1` is less than or equal to `N2`
- guess what `gt` and `ge` do based on the above pattern

To test strings:

- `-n STRING`: if string length is nonzero
- `-z STRING`: if string length is zero
- `S1 = S2`: if strings S1 and S2 are equal
...
Using a loop like this, you can write a simple command-line argument parser that handles options.
